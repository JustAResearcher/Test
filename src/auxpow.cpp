...
const uint256 CMerkleTx::ABANDON_HASH(uint256{1});

static bool AuxpowError(const char* msg)
{
    LogDebug(BCLog::VALIDATION, "AuxPow: %s\n", msg);
    return false;
}
...
void CMerkleTx::InitMerkleBranch(const CBlock& block, int posInBlock)
{
    ...
    nIndex = posInBlock;
    vMerkleBranch = TransactionMerklePath(block, nIndex);
}
...
{
    if (nIndex != 0)
        return AuxpowError("AuxPow is not a generate");

    if (params.fStrictChainId && parentBlock.nVersion.GetChainId () == nChainId)
        return AuxpowError("Aux POW parent has our chain ID");

    if (vChainMerkleBranch.size() > 30)
        return AuxpowError("Aux POW chain merkle branch too long");
    ...
    if (CheckMerkleBranch(GetHash(), vMerkleBranch, nIndex)
        ...
        return AuxpowError("Aux POW merkle root incorrect");

    // Check that there is at least one input.
    if (tx->vin.empty())
        return AuxpowError("Aux POW coinbase has no inputs");
    ...
    if (pc == script.end())
        return AuxpowError("Aux POW missing chain merkle root in parent coinbase");
    ...
    if (pcHead != script.end())
    {
        ...
        if (script.end() != std::search(pcHead + 1, script.end(), std::begin(pchMergedMiningHeader), std::end(pchMergedMiningHeader)))
            return AuxpowError("Multiple merged mining headers in coinbase");
        if (pcHead + sizeof(pchMergedMiningHeader) != pc)
            return AuxpowError("Merged mining header is not just before chain merkle root");
    }
    else
    {
        ...
        if (pc - script.begin() > 20)
            return AuxpowError("Aux POW chain merkle root must start in the first 20 bytes of the parent coinbase");
    }
    ...
    pc += vchRootHash.size();
    if (script.end() - pc < 8)
        return AuxpowError("Aux POW missing chain merkle tree size and nonce in parent coinbase");
    ...
    if (nSize != (1u << merkleHeight))
        return AuxpowError("Aux POW merkle branch size does not match parent coinbase");

    uint32_t nNonce;
    memcpy(&nNonce, &pc[4], 4);
    nNonce = le32toh_internal(nNonce);
    if (nChainIndex != getExpectedIndex(nNonce, nChainId, merkleHeight))
        return AuxpowError("Aux POW wrong index");
