...
FUZZ_TARGET(ephemeral_package_eval, .init = initialize_tx_pool)
{
    ...
    for (const auto& outpoint : g_outpoints_coinbase_init_mature) {
        ...
        outpoints_value[outpoint] = 5000 * COIN;
    }
    ...
    LIMITED_WHILE(fuzzed_data_provider.remaining_bytes() > 0, 300)
    {
        ...
        if (fuzzed_data_provider.ConsumeBool()) {
            ...
            const auto delta = fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(-5000 * COIN, +5000 * COIN);
...
FUZZ_TARGET(tx_package_eval, .init = initialize_tx_pool)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.remaining_bytes() > 0, 300)
    {
        ...
        if (fuzzed_data_provider.ConsumeBool()) {
            client_maxfeerate = CFeeRate(fuzzed_data_provider.ConsumeIntegralInRange<CAmount>(-1, 5000 * COIN), 100);
        }
